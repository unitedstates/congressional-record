...
def parse_legislators(afile,append=False,idstart=0):
    ...
    bio_fields = ['bioguideid','dob','gender','religion','cspan',
                  ...
                  'name_last','official_full','name_middle',
                  'name_suffix','name_nickname']
    ...
    bio_writer = csv.DictWriter(leg_bio, fieldnames=bio_fields, 
                                delimiter='|')
    term_writer = csv.DictWriter(leg_terms, fieldnames=term_fields, 
                                 delimiter='|')
    fec_writer = csv.DictWriter(leg_fec, fieldnames=fec_fields, 
                                delimiter='|')
    idn = idstart
    for leg in leg_yaml:
        bio_row = {}
...
if __name__ == '__main__':
    ...
    past = parse_legislators(os.path.join(YAML_DIR,
                                          'legislators-historical.yaml'),
                                          append=True,idstart=tid)
    
    conn = pg.connect("dbname = congress")
    cur = conn.cursor()
    for table, fpath in [('leg_bio', os.path.join(AUXDATA_DIR, 'leg_bio.csv')),
                        ('leg_terms', os.path.join(AUXDATA_DIR, 
                                                   'leg_terms.csv')),
